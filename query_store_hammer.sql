...
BEGIN
    ...
    SELECT base.[query_id], bucket.*, base.[query_sql_text], p.[query_plan], base.[object_name]
      FROM (SELECT p.[query_id], q.[object_id], COALESCE(OBJECT_NAME(q.[object_id]), '') [object_name], qt.[query_sql_text],
     ...
     CROSS APPLY (SELECT rs.[plan_id], rs.[execution_type_desc] [execution_type], SUM(rs.[count_executions]) [count_executions],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_logical_io_reads] * rs.[stdev_logical_io_reads] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_logical_io_reads] * rs.[count_executions]), 0)), 2), 0) [variation_logical_io_reads],
                         ...
                         ROUND(CONVERT(float, SQRT(SUM(rs.[stdev_logical_io_writes] * rs.[stdev_logical_io_reads] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0))) * 8, 2) [stdev_logical_io_writes],
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_logical_io_writes] * rs.[stdev_logical_io_writes] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_logical_io_writes] * rs.[count_executions]), 0)), 2), 0) [variation_logical_io_writes],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_log_bytes_used] * rs.[stdev_log_bytes_used] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_log_bytes_used] * rs.[count_executions]), 0)), 2), 0) [variation_log_bytes_used],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_query_max_used_memory] * rs.[stdev_query_max_used_memory] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_query_max_used_memory] * rs.[count_executions]), 0)), 2), 0) [variation_query_max_used_memory],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_physical_io_reads] * rs.[stdev_physical_io_reads] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_physical_io_reads] * rs.[count_executions]), 0)), 2), 0) [variation_physical_io_reads],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_rowcount] * rs.[stdev_rowcount] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_rowcount] * rs.[count_executions]), 0)), 2), 0) [variation_rowcount],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_tempdb_space_used] * rs.[stdev_tempdb_space_used] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_tempdb_space_used] * rs.[count_executions]), 0)), 2), 0) [variation_tempdb_space_used],
                         ...
                         ISNULL(ROUND(CONVERT(float, (SQRT(SUM(rs.[stdev_clr_time] * rs.[stdev_clr_time] * rs.[count_executions]) / NULLIF(SUM(rs.[count_executions]), 0)) * SUM(rs.[count_executions])) / NULLIF(SUM(rs.[avg_clr_time] * rs.[count_executions]), 0)), 2), 0) [variation_clr_time]
                    FROM sys.query_store_runtime_stats rs
                    JOIN sys.query_store_plan p
                      ON p.[plan_id] = rs.[plan_id]
