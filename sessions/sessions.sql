           ...
           CASE
             WHEN ((SELECT ISNULL(NULLIF(SUBSTRING([text], (sub.[statement_start_offset] / 2) + 1, (CASE
                 ...
                 (SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                         ...
                         REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                         N'--' + NCHAR(13) + NCHAR(10) + t.[text] +
                         NCHAR(13) + NCHAR(10) + N'--' COLLATE Latin1_General_Bin1,
                         NCHAR(31),N'?'),NCHAR(30),N'?'),NCHAR(29),N'?'),NCHAR(28),N'?'),NCHAR(27),N'?'),NCHAR(26),N'?'),NCHAR(25),N'?'),NCHAR(24),N'?'),NCHAR(23),N'?'),
                         NCHAR(22),N'?'),NCHAR(21),N'?'),NCHAR(20),N'?'),NCHAR(19),N'?'),NCHAR(18),N'?'),NCHAR(17),N'?'),NCHAR(16),N'?'),NCHAR(15),N'?'),NCHAR(14),N'?'),
                         NCHAR(12),N'?'),NCHAR(11),N'?'),NCHAR(8),N'?'),NCHAR(7),N'?'),NCHAR(6),N'?'),NCHAR(5),N'?'),NCHAR(4),N'?'),NCHAR(3),N'?'),NCHAR(2),N'?'),NCHAR(1),N'?'),
                         NCHAR(0),N'?'), N'<?', N'??'), N'?>', N'??') AS [processing-instruction(query)]
                    FROM sys.dm_exec_cursors(sub.[session_id]) cur
                   CROSS APPLY sys.dm_exec_sql_text(cur.[sql_handle]) t
                     FOR XML PATH(''), TYPE)
             ELSE (SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                          REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                          REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                          N'--' + NCHAR(13) + NCHAR(10) +
                          ISNULL(NULLIF(SUBSTRING([text], (sub.[statement_start_offset] / 2) + 1, (CASE
                                                                                                     WHEN sub.[statement_end_offset] = -1 THEN DATALENGTH([text])
                                                                                                     ELSE sub.[statement_end_offset]
                                                                                                   END - sub.[statement_start_offset]) / 2 + 1), ''), [text]) +
                          NCHAR(13) + NCHAR(10) + N'--' COLLATE Latin1_General_Bin1,
