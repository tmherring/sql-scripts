...
SELECT sub.[session_id], sub.[state], sub.[percent_complete], sub.[login], sub.[duration], sub.[estimated_completion_duration], sub.[host_name], 
  ...
  FROM (SELECT DISTINCT s.[session_id], s.[is_user_process],
         ...
         CROSS APPLY sys.dm_exec_input_buffer(s.[session_id], r.[request_id]) deib
         WHERE (COALESCE(r.[blocking_session_id], 0) <> 0
            OR (CASE
                  WHEN r2.[session_id] IS NOT NULL AND (r.[blocking_session_id] = 0 OR r.[session_id] IS NULL) THEN '1'
                  ELSE NULL
                END) IS NOT NULL)
            OR DB_NAME(s.database_id) = 'repositorydb') sub
 ORDER BY sub.[head_blocker] DESC, [blocking_session];
